...
- ブラウザの開発者ツール（Console・Network タブ）でエラーや通信を確認できる
- エラーメッセージを Gemini CLI に渡してデバッグを進めることができる
...
今日扱う `server.js` は後者。Node.js というランタイム（JavaScript の実行環境）の上で動くサーバープログラム。

##### API とは何か・なぜ必要か

**API（Application Programming Interface）** ＝ プログラム同士のやりとりの取り決め。

Web API の場合、「どの URL に・どんな方法（GET / POST など）で頼むと・どんな形式（JSON など）で返ってくるか」という契約。
...
| `/admin` | GET | 管理ページを返す | HTML（ログイン時のみ） |
...
ブラウザ（誰でも触れる）
    ↓ HTTP リクエスト（API 経由）
サーバー（API として公開された操作だけ受け付ける）
    ↓ SQL（自分だけが知っている）
データベース（中身そのもの・直接公開しない）
```
...
Day4 と同じ構成のアプリを、自分でテーマを決めてもう一度作る。テーマは自由。
...
> ここだけは Gemini CLI では実行できない。サーバーは起動し続ける必要があるため、Gemini CLI とは別のターミナルタブで実行する。

▶ **別ターミナルタブを開く**

- VSCode の統合ターミナル下部の `+` ボタンをクリックして新しいターミナルタブを開く
- （Gemini CLI が動いているタブとは別のタブになる）
...
- 追加した機能がブラウザで動作すること
- `git log --oneline` を Gemini CLI に頼んで実行し、コミットが増えていること
...
エラーが出たときに「何が起きているか」を確認する方法を実践する。

#### ブラウザの開発者ツール

F12（Mac は `Cmd + Option + I`）で開発者ツールを開く。

**Console タブ**：

- JavaScript のエラーが表示される
- fetch が失敗したときのメッセージが出る
- `console.log` で出力した値を確認できる

**Network タブ**：

- fetch でどの URL にリクエストを送ったか確認できる
- レスポンスのステータスコード（200 / 404 / 500 など）が確認できる
- Response タブでサーバーが返した JSON の中身を確認できる

確認のポイント：

```
赤く表示されたリクエスト → エラーが起きている
ステータス 404 → URL が間違っている
ステータス 500 → サーバー側でエラーが起きている（ターミナルを確認）
```

#### Node.js のエラーメッセージを読む

ターミナルに出るエラーの読み方：

```
Error: Cannot find module 'express'     ← エラーの種類と内容
    at Function.Module._resolveFilename  ← どこで起きたか（スタックトレース）
```
...
「何のエラーか」だけでなく「なぜそのエラーが出るのか」も聞くと理解が深まる。
...
```
今の状態（自分だけ）：
  自分のPC → localhost:3000

デプロイ後（誰でも使える）：
  誰でも → https://my-vote-app.onrender.com
```
...
Render（クラウドでサーバーを起動）
    ↓
https://my-app.onrender.com（外部からアクセスできる URL）
```
...
3. Repository name を入力（例：`vote-app`）
4. 「Public」を選択
5. 「Create repository」をクリック
6. 作成後に表示される URL をコピーする（例：`https://github.com/username/vote-app.git`）

**ローカルのリポジトリをプッシュする**

VSCode の統合ターミナルで実行する：

```bash
git remote add origin https://github.com/username/vote-app.git
git branch -M main
git push -u origin main
```

- `git remote add origin ...`：リモートリポジトリ（GitHub）を「origin」という名前で登録する
- `git push -u origin main`：ローカルの `main` ブランチをリモートに送る

GitHub のリポジトリページを開いて、コードが表示されていることを確認する。
...
2. 「Continue with GitHub」でサインイン（GitHub アカウントと連携）
3. ダッシュボードが表示されたら準備完了

#### Web Service を作成する

1. ダッシュボードの「New +」→「Web Service」をクリック
2. 「Connect a repository」→ 先ほど作成した `vote-app` リポジトリを選択
3. 以下のように設定する：
...
5. デプロイが始まる（2〜5 分程度かかる）

#### デプロイ結果を確認する

デプロイが完了すると `https://vote-app-xxxx.onrender.com` のような URL が表示される。
...
```
ブラウザ ──①POST /login（user, pass）──→ サーバー
                                       ↓ 認証成功
                               セッションを作成（サーバー側に保持）
                               セッションIDを Cookie でブラウザに送信

ブラウザ ──②GET /admin（Cookie 自動添付）──→ サーバー
                                       ↓ Cookie からセッションID取得
                               セッションあり → 管理ページを返す
...
| `POST /login` | 入力されたユーザー名・パスワードを検証し、正しければセッション開始＋ `/admin` へリダイレクト |
| `GET /admin` | セッション有 → 管理ページを返す。無 → `/login` へリダイレクト |
| `POST /logout` | セッションを破棄して `/login` へ戻す |

加えて、既存の集計リセット処理を `POST /admin/reset` として「セッションがないと使えない」エンドポイントに変える。
...
```
以下の機能を server.js に追加して。

1. express-session パッケージを使ってセッション管理を設定する
   - セッションシークレットは .env の SESSION_SECRET から読み込む

2. 管理者の認証情報は .env から読み込む
   - ユーザー名：ADMIN_USER
   - パスワード：ADMIN_PASS

3. GET /login：ユーザー名・パスワードの入力フォームを含む HTML ページを返す

4. POST /login：
   - フォームから送られたユーザー名・パスワードを確認する
   - 正しければセッションを開始して /admin にリダイレクト
   - 違えばエラーメッセージをつけて /login に戻す

5. GET /admin：
   - セッションが有効なら管理ページ（「投票をリセット」ボタン・ログアウトボタン）を返す
   - セッションが無効なら /login にリダイレクト

6. POST /admin/reset：
   - セッションが有効なら votes テーブルを全件削除して /admin にリダイレクト
   - セッションが無効なら /login にリダイレクト

7. POST /logout：セッションを破棄して /login にリダイレクト
...
| ② 正しい認証情報でログインできる | ユーザー名 `admin` / パスワード `password123` を入力して送信 | `/admin` にリダイレクトされ、管理ページが表示される |
| ③ 管理ページから集計をリセットできる | 「投票をリセット」ボタンをクリック | 集計がゼロになる |
| ④ ログアウトできる | ログアウトボタンをクリック | `/login` に戻る |
| ⑤ 未ログインで /admin に直接アクセスできない | ログアウト後に `http://localhost:3000/admin` を開く | `/login` にリダイレクトされる |
...
- ログインフォームが表示される
- 正しい認証情報でログインすると管理ページに進める
- 未ログインで `/admin` に直接アクセスすると `/login` にリダイレクトされる
- GitHub のリポジトリに `.env` ファイルが表示されていないこと（誤って push されていないか確認）

---

### チャレンジ③：管理ページに機能を 1 つ追加してみよう【個人演習・10分】

セクション 7 で作った管理ページに、**自分で考えた機能を 1 つ追加する**。セッションによる保護は既にあるので、管理ページの中身を拡張することに集中する。

**やること（以下から選ぶか、自分でアイデアを出す）**

| アイデア例 | 内容 |
| --- | --- |
| 投票一覧を表示する | `voted_at` のタイムスタンプつきで全投票を管理ページに一覧表示する |
| 選択肢ごとにリセットボタンを置く | 全体リセットだけでなく、特定の選択肢だけ削除できるボタンを追加する |
| 現在のログイン中ユーザー名を表示する | 管理ページの右上などに「ログイン中：admin」と表示する |

**進め方のルール**

1. Gemini CLI に「〇〇を追加したい。どう実装すればいい？まだ変更しないで。」と聞く
2. 提案を確認してから実装を依頼する
3. ブラウザで動作確認する
4. コミットする
...
- 追加した機能が管理ページ（ログイン後）で動作すること
- 未ログイン状態ではその機能にアクセスできないこと
...
③ ブラウザの開発者ツール（Console / Network）でエラーや通信を確認できた
④ GitHub にプッシュできた
⑤ Render にデプロイして外部 URL でアクセスできた
⑥ 認証なしで公開することのリスクを説明できる
⑦ /login でログインして /admin に進み、集計をリセットできた
...
ブラウザ（index.html）
    ↓ fetch
server.js（Node.js + Express）← .env でログイン情報・セッション管理
    ↓ SQL
votes.db（SQLite）
    ↑ デプロイ済み（Render）
/login・/admin ページで管理者のみ操作可能
```
...
| 86〜95分  | 17:06〜17:15 | **チャレンジ③：管理ページに機能を 1 つ追加してみよう**                   | 個人演習       | 同様に実施           |
...
- Git コミット履歴（`git log --oneline` のキャプチャ）
- GitHub リポジトリの URL
- Render にデプロイした外部 URL と、そこにアクセスした画面キャプチャ
- ログインページ（`/login`）と管理ページ（`/admin`）の画面キャプチャ
