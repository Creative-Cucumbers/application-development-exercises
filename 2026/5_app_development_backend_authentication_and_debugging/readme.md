...
アイスブレイクのテーマ：「この連休（GW）でいちばん良かったことは?」

- 現地・リモートともに Slack に投稿する
- 講師がいくつかピックアップして読み上げる

---

#### 最近のAIニュース【共有・3分】

授業を始める前に、ここ最近の AI 関連ニュースを 2 つ紹介する。

**① OpenAI Codex：AI がパソコン自体を操作する（2026年4月16日）**

OpenAI が「Codex for almost everything」として Codex アプリの大規模アップデートを発表。目玉は **background computer use（AI がパソコンを直接操作する機能）**。

これまで AI コーディングエージェントは「コードを書いてくれる」ものだった。今回の更新で Codex は：

- **自分のカーソルで画面を見て、クリックして、文字を入力できる**
- 自分が別の作業をしている間に、**複数のエージェントが Mac 上で並行して動ける**
- フロントエンドの修正・アプリのテスト・API を持たないアプリの操作も可能
- 90 以上のプラグイン・アプリ内ブラウザ・画像生成機能も追加

「コードを書いてくれる AI」から「パソコンをまるごと代わりに操作してくれる AI」への進化。

> Day4 から使っている Gemini CLI は「チャットで依頼する」タイプ。Codex の新機能は「画面を見て自分でクリックする」タイプ。AI の作業スタイルの幅が大きく広がっている。

▶ **デモ動画（1分30秒）**：[OpenAI Codex — Computer Use](https://www.youtube.com/watch?v=D_FCYsshMI4)

---

**② Anthropic：Claude Code の利用上限倍増と SpaceX との大型計算資源契約（2026年5月6日）**

Anthropic が Claude Code（今日使っている Gemini CLI と同種の AI コーディングエージェント）の利用上限を大幅に引き上げると発表。

- Pro・Max・Team・Enterprise プランで、**5時間あたりの使用上限が 2 倍**に
- ピーク時間帯の制限も撤廃
- Claude Opus の API 利用上限も大幅引き上げ（入力トークン：3万 → 50万 / 分）

背景には **SpaceX との大型計算資源契約** がある。SpaceX の「Colossus 1」データセンターの計算能力（**300 MW 以上・GPU 22 万台超**）を丸ごと借りることで、Claude を動かす「計算の余裕」を一気に確保した。今後は宇宙空間での計算基盤整備も検討しているという。

> AI サービスの裏側には膨大な計算能力が必要。「ロケットで有名な SpaceX」が「AI 企業の Anthropic」に計算資源を貸す、という異業種連携が話題になった。私たちが使う Gemini CLI（Google）と Claude Code（Anthropic）は現在、AI コーディングエージェント市場で激しく競争している。
