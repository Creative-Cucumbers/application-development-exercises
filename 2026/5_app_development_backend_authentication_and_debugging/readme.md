...
3. エクスプローラーに `.git` フォルダが作成されたことを確認する（表示されない場合は `ls -la` で確認）
...
Public リポジトリにプッシュしたファイルは、**世界中の誰でも読める**。
...
> プッシュした後に「`.env` を上げてしまった」と気づいても、削除コミットをしただけでは履歴から消えない。GitHub のサポートに問い合わせて履歴ごと削除してもらう必要があり、手間がかかる。パスワードは即座に変更すること。

#### GitHub アカウントを作成する（まだ持っていない人）

Google アカウント（Gmail）を使って登録する。

1. https://github.com にアクセスして「Sign up」をクリック
2. **Enter your email** に Gmail のアドレスを入力して「Continue」
3. **Create a password** にパスワードを入力して「Continue」（Gmail のパスワードと同じにする必要はない）
4. **Enter a username** にユーザー名を入力して「Continue」（例：`yamada-taro`）
5. メール通知の設定は「n」を入力して「Continue」でスキップ
6. ロボット認証（パズル）を解く
7. 入力した Gmail に確認メールが届くので、6桁のコードを **Enter code** 欄に入力
8. 「Free」プランを選択してアカウント作成完了
...
**方法 A：VSCode の GUI から行う（推奨）**

🖱 **VSCode**

1. Source Control パネルを開く（`Cmd + Shift + G` / `Ctrl + Shift + G`）
2. パネル上部の「…」→「リモート」→「リモートの追加」をクリックする
3. 先ほどコピーした GitHub のリポジトリ URL（例：`https://github.com/username/vote-app.git`）を入力する
4. リモート名は `origin` のまま Enter を押す
5. パネル上部の「…」→「プッシュ」をクリックする
6. 「ブランチを発行しますか？」と聞かれたら「OK」を押す

---

**方法 B：Gemini CLI から行う**

💬 **Gemini CLI**

```
以下のリモートリポジトリを登録して、コードをプッシュして。
URL：https://github.com/username/vote-app.git
```

Gemini CLI がコマンドを提案するので、確認して承認する。

---

🖱 **VSCode（確認）**

GitHub のリポジトリページをブラウザで開き、`server.js` や `index.html` などのファイルが表示されていることを確認する。
...
#### Render に GitHub を連携する

初回デプロイの前に、Render と GitHub を連携させる。

1. ダッシュボード右上のアカウントメニュー →「**Account Settings**」を開く
2. 「**Connected Accounts**」→「**Connect GitHub**」をクリック
3. GitHub のログイン画面が表示されるのでサインイン
4. 「Authorize Render」をクリック
5. 「Install Render」の画面で「All repositories」または「Only select repositories」を選んで「Install」をクリック
6. Render のダッシュボードに戻り、GitHub の連携が表示されていれば完了
...
```
入場時：チケットを見せる → スタッフが確認 → リストバンドを渡す
    ↓
会場内：チケットは不要。リストバンドを見せるだけで入れる
    ↓
退場（ログアウト）：リストバンドを外す → 以降は入れなくなる
```
