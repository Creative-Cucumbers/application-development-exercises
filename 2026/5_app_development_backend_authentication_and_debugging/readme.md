...
```mermaid
graph TD
    A["ブラウザ（index.html）"] -->|"POST /vote"| B["server.js（Node.js + Express）"]
    A -->|"GET /votes"| B
    B -->|"INSERT INTO votes"| C[("votes.db（SQLite）")]
    B -->|"SELECT option, COUNT"| C
    C --> B
    B --> A
```
...
デプロイ後（誰でも使える）：
  誰でも → https://app-dev-day5-xxxx.onrender.com
```
