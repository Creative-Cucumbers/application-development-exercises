...
教員が数件ピックアップしてコメント → 「JavaScript の変数はメモリ上にある、ページを閉じると消える」をまとめる。
...
回答をいくつか紹介しながら「どのアプリも、データをどこかに書き込んで残している」（＝永続化）につなぐ。
...
flowchart TD
    A["🌐 ブラウザ（index.html）"]
    B["⚙️ Node.js サーバー（server.js）\n← 今日 Gemini CLI で作る"]
    C[("🗄️ votes.db\nSQLite ファイル")]

    A -- "fetch('/vote') でリクエストを送る" --> B
    B -- "SQL でデータを操作する" --> C
```

**server.js の最小構造（概要のみ・5分は不要・流れの説明のみ）**
