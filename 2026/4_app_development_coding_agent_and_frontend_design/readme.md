# Day4 アプリ開発発展①：バイブコーディングの振り返り・コーディングエージェントと開発環境・データ

![Day4 表紙](./assets/ChatGPT%20Image%202026年5月7日%2008_49_14.png)
...
**① AI の性能がさらに向上：テキストも画像も「考えてから生成する」時代へ**

4月だけで OpenAI・Anthropic から相次いでアップデートが発表された。

- **GPT-5.5（4月23日）**：OpenAI が発表。発表資料の中で「vibe coding」という言葉を公式に使用。コードを書く・デバッグする・ツールをまたいで作業を完結させる能力が向上　[→ OpenAI 発表](https://openai.com/index/introducing-gpt-5-5/) / [→ TechCrunch](https://techcrunch.com/2026/04/23/openai-chatgpt-gpt-5-5-ai-model-superapp/)
- **Claude Opus 4.7（4月16日）**：Anthropic が発表。コーディング・エンジニアリング性能が前バージョンから大幅に向上。長時間・複雑なエージェントタスクへの対応が強化された　[→ GitLab ブログ](https://about.gitlab.com/blog/claude-opus-4-7-is-now-available-in-gitlab-duo-agent-platform/)
- **GPT Image 2.0（4月21日）**：OpenAI が発表。画像生成モデルが初めて「考えてから描く（Thinking）」機能を搭載。日本語を含む文字の描写が劇的に向上し、インフォグラフィックや UI 要素の生成も可能に　[→ OpenAI 発表](https://openai.com/index/introducing-chatgpt-images-2-0/) / [→ TechCrunch](https://techcrunch.com/2026/04/21/chatgpts-new-images-2-0-model-is-surprisingly-good-at-generating-text/)

「テキスト生成・コード生成・画像生成」のいずれも、AI が推論したうえで出力する方向に進んでいる。

**② 学生向け無料プランの動向：AI の普及速度が利用規約を変える**

企業によっては学生に無料でコーディングツールを提供しているケースがある（例：GitHub Copilot の学生プラン、Gemini の無料枠など）。ただし AI の計算コストの急増により、内容は変わる可能性がある。

- **GitHub Copilot 学生プランが新規登録を一時停止（4月20日）**：コーディングエージェントの普及でインフラコストが急増し、新規の学生登録を停止。既存ユーザーは継続可能　[→ GitHub Blog](https://github.blog/jp/2026-04-21-changes-to-github-copilot-individual-plans/) / [→ GitHub Community](https://github.com/orgs/community/discussions/189268)
- 今日使う **Gemini CLI は Google アカウントがあれば無料枠で利用可能**だが、同様に動向には注意が必要
- 「今日使えているツールが、来月も同じ条件で使えるとは限らない」という前提で AI ツールと付き合う姿勢が大切
...
2. **`.git` フォルダが作られたことを確認する**
   ...
   **方法 A：ターミナルで確認する**
   ...
   `.git` というフォルダが表示されれば初期化成功。

   **方法 B：VSCode の Explorer に表示する**

   デフォルトでは `.git` は非表示になっている。設定を変えると Explorer に表示できる。

   1. メニューから `Code（Mac）/ File（Windows）` → `基本設定` → `設定` を開く（ショートカット：`Cmd + ,` / `Ctrl + ,`）
   2. 検索欄に `files.exclude` と入力する
   3. リストに `**/.git` という項目が表示される
   4. その項目の左にある鉛筆アイコンをクリックし、`項目の削除` を選択する
   5. VSCode Explorer のサイドバーに `.git` フォルダが表示されることを確認する

   確認が終わったら設定を元に戻してもよい（同じ手順で `**/.git` を追加する）。中のファイルは直接編集しないこと。
...
```
5.5フォルダを作成し、その中で自己紹介ページを HTML/CSS で作って。
...
- Mac：Explorer で `hello.html` を右クリック → `Finder で表示`（Reveal in Finder）→ ファイルをダブルクリック
- Windows：Explorer で `hello.html` を右クリック → `エクスプローラーで表示`（Reveal in File Explorer）→ ファイルをダブルクリック
...
- Day2・Day3 の micro:bit センサー値も非構造化データ。AI が「激しく動いている」などの状態に変換できる
...
#### 構造化データをどこに持つか（MECE 整理）

構造化データを扱う方法は大きく 4 つに分けられる。

| 方法 | 具体例 | 残るか | 大量データの検索 | 利用用途例 |
| ---- | ------ | ------ | ---------------- | ---------- |
| **メモリ（変数）** | JavaScript の配列・オブジェクト | ✗ 消える | - | フォームの入力値・UI の一時状態 |
| **ファイル** | JSON / CSV / テキストファイル | ✓ 残る | 不向き | 設定ファイル・ログ保存・小規模なデータエクスポート |
| **RDB**（リレーショナルDB） | SQLite / MySQL / PostgreSQL | ✓ 残る | SQL で効率よく操作できる | 会員情報・注文履歴・投票データ・商品カタログ |
| **NoSQL DB** | MongoDB / Redis / Firebase | ✓ 残る | 用途に応じて使い分ける | チャット履歴・セッション管理・リアルタイムデータ |

![構造化データの持ち方の4分類](./assets/structured_data_storage_options.svg)

1 コマ目で作った投票アプリは「メモリ」に保存していたため、リロードで消えた。今日はこれを **RDB** で永続化する。

**RDB（リレーショナルデータベース）とは**

- データを「テーブル（表）」で管理し、行と列の構造を持つ
- テーブル同士を「関係（リレーション）」で結び付けて複雑なデータを扱える
- SQL という言語でデータを操作する
- 代表例：MySQL・PostgreSQL・SQLite

**SQLite は RDB の一種**

SQLite は RDB のしくみをそのまま持ちながら、データベース全体を **1 つの `.db` ファイル** に収めたもの。サーバーを立てなくても使えるため、学習・プロトタイプ・小規模アプリに向いている。
...
### 11A. SQLite 実践（ローカル）【実習】

ローカルに `votes.db` ファイルを作成し、投票アプリが使うテーブルを実際に操作する。**ファイルとして手元に残ること**を VSCode で確認するのがこの実習のポイント。

#### 準備：フォルダの作成と sqlite3 の確認
...
```
11.0-A というフォルダを作って。SQLite の練習用フォルダです。
```

VSCode で `ファイル`（File）→ `フォルダーを開く`（Open Folder）から `11.0-A` フォルダを開く。
...
- Mac：通常プリインストール済み。バージョンが表示されれば OK
- Windows：表示されない場合は https://www.sqlite.org/download.html から `sqlite-tools-win-x64-*.zip` をダウンロードして展開し、パスを通す

> **インストールがうまくいかない場合**：https://sqliteonline.com をブラウザで開いて同じ SQL を実行する。ファイルとしては残らないが、SQL の操作は同じように体験できる。

#### フェーズ 1：全員で一緒に実行する

ターミナルで以下を実行して `votes.db` を作成・起動する：

```bash
sqlite3 votes.db
```

`sqlite>` というプロンプトが表示されたら準備完了。以降の SQL をここに入力する。
...
#### votes.db の実体を VSCode で確認する

sqlite3 を終了すると、`11.0-A` フォルダに `votes.db` ファイルが生成されている。

1. VSCode Explorer に `votes.db` が表示されていることを確認する
2. VSCode 拡張機能マーケットプレイスで「SQLite Viewer」を検索してインストールする
3. Explorer で `votes.db` をクリックすると、テーブルと行がGUI で表示される

→ 「SQL で操作したデータがファイルに残っている」ことを視覚で確認する。

#### フェーズ 2：Gemini CLI でクエリを試す

ターミナルで Gemini CLI を起動し、以下を依頼する：

```
11.0-Aフォルダに
直近 5 件の投票を新しい順に取得する SQL を書いて。
votes テーブルのカラムは id, option, voted_at。
query.sql というファイルに保存して。
```

生成された `query.sql` をターミナルで実行する：

```bash
sqlite3 votes.db < query.sql
```

「Gemini が書いた SQL がそのままローカルの DB に対して動く」ことを確認する。
...
```
11.0-Bというフォルダを作って。Node.js のサーバープロジェクト用のフォルダです。
また、6.0フォルダからindex.htmlをコピーし、11.0-Bに配置してください。
他のファイルはまだ作れないでください。
```

フォルダが作成されたら、VSCode で `ファイル`（File）→ `フォルダーを開く`（Open Folder）から `11.0-B` フォルダを開き直す。
以降の作業はすべてこのフォルダの中で行う。
...
```
11.0-Bのindex.htmlの投票アプリですが、リロードするとデータが消えてしまう。
Node.js と Express と better-sqlite3 を使って、投票アプリのバックエンドを作って。
...
![server.js が index.html と votes.db をつなぐ流れ](./assets/server_runtime_flow.svg)

#### ステップ 4：index.html を修正する（10分）
